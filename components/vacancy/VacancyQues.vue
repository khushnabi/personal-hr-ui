<template>
  <div v-loading.fullscreen.lock="fullscreenLoading">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item :to="{path: '/vacancy'}">Vacancies</el-breadcrumb-item>
    ...
    <el-card class="card-box">
      <div slot="header">
        <span>Question</span>
        <el-button @click="addQuestion" class="addSkills" type="primary" size="small">
  ...
  methods: {
    ...
    createQuestion(formName) {
      this.$refs[formName].validate(async valid => {
        if (valid) {
          this.questions = await Axios.post(
            `/vacancies/${this.id}/questions`,
            this.question
          );
          this.question = {};
          this.fetchVacancy();
    ...
    async updateQues() {
      this.fullscreenLoading = true;
      this.questions = await this.vacancy.UpdateQuestio(
        this.filterQuestion.id,
        this.question
      );
      this.fetchVacancy();
      this.fullscreenLoading = false;
